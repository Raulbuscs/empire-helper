...
function setBuffValue(item) {
    ...
    try {
        ...
        console.log('error empirePrice methoth one: ', e)
        try {
            empirePrice_full = item.querySelector('.font-numeric.inline-flex span:nth-of-type(2)').innerText.replace(',', '');
        } catch (error) {
            console.log('error empirePrice methoth two: ', error)
    ...
    if (empirePrice_full != undefined) {
        ...
        if (auto_trader) {
            let timer_name = 'trade_check_' + Math.random().toString(36).substring(2, 15)
            console.time(timer_name);

            if (trade_custom_filter) {
                if (calc <= max_markt && rollpercent_calc < (1 + (max_roll / 100)) && do_not_withdraw_short.length < 10) {
                    if (!isSticker && !itemName.includes('★ StatTrak™') && !itemName.includes('Souvenir')) {
                        if (!do_not_withdraw.some((v) => itemName.includes(v)) && !excludeItems.some((v) => itemName.includes(v))) {
                            console.timeEnd(timer_name);
                            let skinbutton = item.querySelector('cw-csgo-market-item-card')
                            addName(itemName);
                            addName_short(itemName);

                            handleNewItem(skinbutton)
                        }
                    }
                }
            }

            else if (trade_buy_list_filter) {
                if (calc <= max_markt && rollpercent_calc < (1 + (max_roll / 100)) && do_not_withdraw_short.length < 10) {
                    if (!itemName.includes('Souvenir')) {
                        if (condition) {
                            console.timeEnd(timer_name);
                            let skinbutton = item.querySelector('cw-csgo-market-item-card')
                            addName_buy_list(itemName);
                            addName_short(itemName);
        ...
        log(blue,`${itemName}`)
        log(yellow,`\t ROLL PRICE: ${empirePrice} coins`)
...
function addName(name) {
    do_not_withdraw.push(name);
    console.log(`Added ${name} to the array`);
    setTimeout(() => {
      removeName(name, do_not_withdraw);
    }, 15 * 60 * 1000); // remove the name after 15 minutes
}

function addName_buy_list(name) {
    do_not_withdraw.push(name);
    console.log(`Added ${name} to the array`);
    setTimeout(() => {
      removeName(name, do_not_withdraw);
    }, 10 * 60 * 1000); // remove the name after 15 minutes
}

function addName_short(name) {
    do_not_withdraw_short.push(name);
    console.log(`Added ${name} to the array`);
    setTimeout(() => {
      removeName(name, do_not_withdraw_short);
    }, 0.5 * 60 * 1000); // remove the name after 15 minutes
}
...
function checkPrice(empirePrice, buffPrice){
    ...
    if (val > 1.00) return "Overpriced";
    if (val <= 1.00 && val >= 0.97) return "Goodpriced";
...
function setBuffValueTradeBar(item) {
    ...
    let exterior = item.querySelector('cw-csgo-market-item-card > div > div > div:nth-child(3) > div:nth-child(1) > span').innerHTML.trim().slice(0, 2)
    console.log('exterior: ', exterior)
    ...
    try {
        empirePrice_full = Math.floor(item.querySelector('cw-pretty-balance > span').innerText.replace(',', '') * 100) / 100;
    } catch (e) {
        console.log('error empirePrice methoth two: ', error)
        empirePrice_full = undefined;
    }
    ...
    if (empirePrice_full != undefined) {
        try {
            ...
            //empirePrice = empirePrice_full / rollpercent_calc
            empirePrice = empirePrice_full

            console.log(rollpercent)
            console.log(rollpercent_calc)
            console.log(empirePrice_full)
            console.log(empirePrice)
        } catch (error) {
            try {
                let rollpercent = item.querySelector('cw-csgo-market-item-card > div > div > div:nth-child(3) > div:nth-child(3) > span').textContent
